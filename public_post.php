...
$obj= new Connections();
 ...
 $sql4= $obj->db->prepare("select * from  wall");
  $sql4->execute();
  while($row4 = $sql4->fetch(PDO::FETCH_ASSOC)){
      $post_id=$row4['ID'];

    
  $sql1 = $obj->db->prepare("select * from  wall where types='Public'");
  $sql1->execute();
...
{ 
 ...
 echo "<script>window.location='public_post.php';</script>";
}
else{
...
{ 
 ...
 echo "<script>alert('Your Comment Posted!!');window.location='public_post.php';</script>";
}
else{
echo "<script type='text/javascript'>
alert('Email is already Register with us!!');

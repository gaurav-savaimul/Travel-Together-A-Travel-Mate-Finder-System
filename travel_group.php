...
<input type="text" name="search" required placeholder="Enter to Search Travel Plan "  class="form-control">
...
<div class="row">
 ...
 if(isset($_POST['search'])){
    $search=$_POST['search'];
          $sql = "SELECT * FROM travel_groups where userid!='$userid' AND group_name like '%$search%'";
}else{
     $sql = "SELECT * FROM travel_groups where userid!='$userid'";
            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    echo '<div class="col-md-4" style="max-height:400px; overflow: hidden;">';
                   ...
                   ?>
                    <a href="view_group_details.php?id=<?php echo $row['ID']; ?>" class="btn btn-primary btn-block">View Details</a><br><br>
                   <?php
                    echo    '</div>';
                }
            } else {

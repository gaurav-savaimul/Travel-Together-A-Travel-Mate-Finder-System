...
<?php	
  ...
  $sql = $obj->db->prepare("select * from answer ORDER BY ID DESC");
		
		$sql->execute();
		
			
		    while($row = $sql->fetch(PDO::FETCH_ASSOC)) {
			$QAR_no=$row['unique_qar_no'];
			$QAR_date_time=$row['date_time'];
			
			?> 
    <?php 
						$id=$row['question_id'];
						 $sql1 = $obj->db->prepare("select * from  question where ID='$id'");
						 $sql1->execute();
						 while($row1 = $sql1->fetch(PDO::FETCH_ASSOC)) {
						$uploaded_by=$row1['uploaded_by'];
						?><tr>
    
     <td><?php echo $QAR_no; ?></td>
       <td><?php echo $QAR_date_time; ?></td>
     <td>
     ...
     <?php echo $uploaded_by; ?>
       ...
       <td>
       <a href="QAR-View-Details.php?id=<?php echo $row['ID']; ?>" class="btn btn-primary"> View </a>
	   </td>
